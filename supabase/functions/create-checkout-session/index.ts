...
serve(async (req) => {
  ...
  try {
    ...
    if (!proPriceId) {
      ...
      return new Response(
        JSON.stringify({ error: "Server configuration error: Stripe Pro price missing." }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    if (requestedPriceId && requestedPriceId !== proPriceId) {
      console.error(`[create-checkout-session] Error: Invalid priceId requested: ${requestedPriceId}`);
      return new Response(
