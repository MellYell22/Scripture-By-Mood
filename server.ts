...
const getSubscriptionSyncData = async (stripe: Stripe, subscriptionId?: string | null) => {
  if (!subscriptionId) {
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        const subscription = await getSubscriptionSyncData(stripe, subscriptionId);
        console.log(`[Server Webhook] Syncing checkout profile ${profile.id} to ${subscription.tier}; price=${subscription.priceId || 'none'} status=${subscription.status || 'none'}`);
      ...
      case 'invoice.payment_succeeded': {
        ...
        const subscription = await getSubscriptionSyncData(stripe, subscriptionId);
        console.log(`[Server Webhook] Confirming subscription for user ${profile.id}: tier=${subscription.tier}, price=${subscription.priceId || 'none'}, status=${subscription.status || 'none'}`);
        const updated = await updateProfileOrThrow(profile.id, {
          stripe_customer_id: customerId,
          stripe_subscription_id: subscriptionId,
          subscription_tier: subscription.tier,
        ...
        console.log(`[Server Webhook] UPDATE SUCCESS: User ${updated.id} tier confirmed as ${updated.subscription_tier}.`);
        break;
      }
      ...
      case 'customer.subscription.updated': {
        ...
        console.log(`[Server Webhook] Syncing subscription ${subscription.id} status: ${status}, price=${priceId || 'none'} for user: ${userIdMetadata}`);
        ...
        console.log(`[Server Webhook] SYNC SUCCESS: User ${updated.id} synchronized to ${updated.subscription_tier}.`);
        break;
      }
